...
class madPdo extends PDO {
    ...
    public $functions = array('avg', 'count', 'max', 'min', 'sum', 'nextval', 'currval', 'concat');
        ...
        foreach ( $tokens as $key => $token ) {
            ...
            foreach( $from as $schemalessTable ) {
                ...
                foreach( $columns as $column ) {
                    ...
                    if ( $querySection == 'from' && in_array( strtolower( $tokens[$key-2] ), array( 'from', 'join', ',' ) ) ) {
                        // avoid bug caused by a table which correspond to 
                        // another table name ...
                        continue 2;
                    }
...
class madPDOStatementWrapper {
    ...
    public function execute( &$input_parameters = array(  ) ) {
        ...
        foreach( $this->objects as $object ) {
            if ( $object->insertAttribute && isset( $input_parameters[$object->insertAttribute] ) ) {
                ...
                $result = $object->execute( $tmp_parameters );
            } else {
